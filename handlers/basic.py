...
async def start_callback(bot, message):
    """ callback for /start (private) """
    uid = get_uid(message)

    await bot.send_message(uid, start_phrase, parse_mode="html")


start_handler = MessageHandler(
    callback=start_callback, filters=Filters.command("start") & Filters.private
)
...
async def help_callback(bot, message):
    ...
    await bot.send_message(uid, help_phrase, parse_mode="html")


help_handler = MessageHandler(callback=help_callback, filters=Filters.private)


async def store_names_callback(bot, message):
